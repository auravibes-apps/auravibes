...
class MessagesController extends _$MessagesController {
  ...
  Future<MessageEntity> _waitMessage({
    ...
    if (mcpServerIds.isNotEmpty) {
      final mcpManager = ref.read(mcpConnectionControllerProvider.notifier);
  ...
  Future<List<ToolSpec>> _buildCombinedToolSpecs(
    ...
    return BuildCombinedToolSpecsUseCase(
  ...
  Future<List<String>> _collectMcpServerIds(
    ...
    return CollectMcpServerIdsUseCase(
      getToolsGroupById: ref
          .read(toolsGroupsRepositoryProvider)
          .getToolsGroupById,
    ).call(enabledTools);
  ...
  Future<(MessageEntity userMessage, MessageEntity systemMessage)> sendMessage({
    ...
    final enabledTools = await ref.read(
      contextAwareToolEntitiesControllerProvider(
        conversationId: conversationId,
