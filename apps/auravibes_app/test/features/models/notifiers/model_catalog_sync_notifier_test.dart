...
void main() {
  group('ModelCatalogSyncNotifier', () {
    test('records attempt and successful sync timestamps', () async {
      ...
      when(() => service.sync()).thenAnswer((_) async {});
    ...
    test('manual failure retains last success and allows another attempt', () async {
      final service = _MockModelSyncService();
      var attempts = 0;
      when(() => service.sync()).thenAnswer((_) async {
        if (attempts++ == 1) throw _serviceUnavailable();
      });
      final container = _containerFor(service);
      addTearDown(container.dispose);
      final notifier = container.read(modelCatalogSyncNotifierProvider.notifier);
    ...
    test('automatic retries transient errors at most three times', () async {
      final service = _MockModelSyncService();
